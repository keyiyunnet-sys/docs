...
<ParamField body="last_image" type="string">
  ...
  <Note>当新旧字段同时传入时，以 `first_image`、`last_image` 为准；若值不一致会报参数冲突</Note>
</ParamField>
...
<ParamField body="referenceImages" type="array">
  ...
  规则说明：
  - 最多 `9` 张
  - 不能与 `first_image`、`last_image` 及兼容字段 `image`、`lastFrameImage` 同时使用
  - 图片建议宽高比在 `0.4 ~ 2.5` 之间，宽高建议在 `300 ~ 6000 px` 之间
...
兼容说明：
- 历史字段 `image`、`lastFrameImage` 仍可兼容使用
- 当新旧字段同时传入时，新字段优先
...
| `sd2_manxue_fast_720p` | `720p` | fast 系列，适合速度优先的常规高清场景 |
| `sd2_manxue_fast_1080p` | `1080p` | fast 系列，适合速度优先的高清输出场景 |
...
<Note>
普通系列 `sd2_manxue_*` 直接按对应分辨率生成；`sd2_manxue_fast_*` 为 fast 系列模型，用户侧仍通过 `model` 字段传模型名，接口会按对应分辨率和 fast 系列能力提交任务。
</Note>
...
curl --request POST \
  ...
  --data '{
    ...
    "prompt": "城市夜景中车辆流光穿梭，镜头快速俯冲后平稳推进",
    "duration": 5,
    "ratio": "16:9"
  }'
```
...
```json 创建失败
{
  ...
  "model": null,
  "status": "failed",
  "error": "参数冲突：referenceImages/referenceVideos/referenceAudios 与 first_image/last_image 不能同时使用"
}
```
