...
<ParamField path="referenceVideos" type="array">
  ...
  - A single video is typically mapped to `video_url`
  - Multiple videos are typically mapped to `video_reference`
...
<ResponseField name="data.projectId" type="string">
  ...
  You can treat this value as the task `id` for subsequent status queries.
</ResponseField>
...
<Info>
**Best Practices:**
1. Video generation is asynchronous, so save the returned task identifier for later queries.
