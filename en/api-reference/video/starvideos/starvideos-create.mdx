...
<ParamField body="image_urls" type="array">
  Single-image input field. Provide `1` image URL for image-to-video generation.

  Rules:
  - No `image_urls`: text-to-video
  - `1` image: single-image-to-video

  <Note>`image_urls` is only for single-image input. Use `first_image` and `last_image` for first/last frame mode.</Note>
</ParamField>

<ParamField body="first_image" type="string">
  First frame image URL for first/last frame mode.
</ParamField>

<ParamField body="last_image" type="string">
  Last frame image URL for first/last frame mode. Must be used together with `first_image`.
</ParamField>

<ParamField body="images" type="array">
  Legacy reference image list.
...
<Info>
**Compatibility notes:**
- The legacy field `images` is still supported
- When both new and legacy fields are provided, the new fields take priority
...
<CardGroup cols={3}>
  <Card title="Text to Video" icon="wand-magic-sparkles">
    Do not provide image input fields. The result is generated only from the prompt.
  </Card>
  <Card title="Single Image" icon="image">
    Provide `image_urls` to strengthen subject and style control.
  </Card>
  <Card title="First/Last Frame" icon="film">
    Provide `first_image` and `last_image` as the start frame and end frame.
  </Card>
</CardGroup>
...
- `image_urls` supports exactly `1` image
...
5. Existing integrations using the legacy `images` field can continue to use it.
</Info>
