...
<ParamField body="last_image" type="string">
  ...
  <Note>When both legacy and current fields are provided, `first_image` and `last_image` take precedence. Conflicting values return a parameter conflict error.</Note>
</ParamField>
...
<Note>
Standard `sd2_manxue_*` models generate directly at the matching resolution. `sd2_manxue_fast_*` models are the fast series. You still pass the public model name through the `model` field, and the API submits the task with the matching fast-series capability.
</Note>
...
curl --request POST \
  ...
  --data '{
    ...
    "prompt": "Traffic light trails crossing a city at night with a quick dive and smooth push-in shot",
    "duration": 5,
    "ratio": "16:9"
  }'
```
...
```json Failure
{
  ...
  "model": null,
  "status": "failed",
  "error": "Parameter conflict: referenceImages/referenceVideos/referenceAudios cannot be used together with first_image/last_image"
}
```
