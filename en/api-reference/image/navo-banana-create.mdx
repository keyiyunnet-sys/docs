...
api: "POST https://zcbservice.aizfw.cn/kyyReactApiServer/v1/image/banana/"
---
...
<ParamField path="resolution" type="string">
    Resolution setting, only supported by `nano-banana-2` and `nano-banana-pro`
...
<RequestExample>
    ...
    ```bash cURL
    curl --request POST \
    --url https://zcbservice.aizfw.cn/kyyReactApiServer/v1/image/banana/ \
    ...
    .uri(URI.create("https://zcbservice.aizfw.cn/kyyReactApiServer/v1/image/banana/"))
    .header("Authorization", "Bearer {{key}}")
    .header("Content-Type", "application/json")
    ...
    'https://zcbservice.aizfw.cn/kyyReactApiServer/v1/image/banana/',
    headers={
        'Authorization': 'Bearer {{key}}',
